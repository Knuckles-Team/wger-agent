...
Installation (prod)
  * [](https://wger.readthedocs.io/en/latest/production/docker.html)
    * [Other setups](https://wger.readthedocs.io/en/latest/production/docker.html#other-setups)
    * [Quickstart](https://wger.readthedocs.io/en/latest/production/docker.html#quickstart)
    * [Configuration](https://wger.readthedocs.io/en/latest/production/docker.html#configuration)
    * [Deployment](https://wger.readthedocs.io/en/latest/production/docker.html#deployment)
    * [Monitoring with grafana](https://wger.readthedocs.io/en/latest/production/docker.html#monitoring-with-grafana)
    * [](https://wger.readthedocs.io/en/latest/production/docker.html#others-common-error-and-pitfalls)
      * [Email verification links](https://wger.readthedocs.io/en/latest/production/docker.html#email-verification-links)
      * [CSRF errors](https://wger.readthedocs.io/en/latest/production/docker.html#csrf-errors)
      * [Wrong pagination links](https://wger.readthedocs.io/en/latest/production/docker.html#wrong-pagination-links)
      * [Missing static files](https://wger.readthedocs.io/en/latest/production/docker.html#missing-static-files)
      * [Automatically start service](https://wger.readthedocs.io/en/latest/production/docker.html#automatically-start-service)
      * [Backup](https://wger.readthedocs.io/en/latest/production/docker.html#backup)
      * [Postgres Upgrade](https://wger.readthedocs.io/en/latest/production/docker.html#postgres-upgrade)
      * [Building the image](https://wger.readthedocs.io/en/latest/production/docker.html#building-the-image)
      * [Using sqlite](https://wger.readthedocs.io/en/latest/production/docker.html#using-sqlite)
...
[wger project](https://wger.readthedocs.io/en/latest/index.html)
  ...
  * Docker compose
  * [ Edit on GitHub](https://github.com/wger-project/docs/blob/master/docs/production/docker.rst)


* * *
# Docker compose[](https://wger.readthedocs.io/en/latest/production/docker.html#docker-compose "Link to this heading")
<https://github.com/wger-project/docker>
The prod docker compose file starts up a production environment with gunicorn as the webserver, postgres as a database and redis for caching with nginx used as a reverse proxy.
...
## Other setups[](https://wger.readthedocs.io/en/latest/production/docker.html#other-setups "Link to this heading")
>

**Kubernetes**

> There is a helm charts repository if you want to deploy the application with kubernetes: <https://github.com/wger-project/helm-charts>

**TrueNAS SCALE**

> Consult this guide if you want to deploy the application to [TrueNAS SCALE](https://wger.readthedocs.io/en/latest/production/true_nas.html#truenas)
## Quickstart[](https://wger.readthedocs.io/en/latest/production/docker.html#quickstart "Link to this heading")
...
The docker image comes with a default set of exercises, but due to the size of the dataset, no ingredients. You can manually sync the datasets from the wger.de instance (or any other) with the following commands:
```
...
The full ingredient dataset is quite larger, taking around 1GB of space in the db and needs a far longer time to download and process:
```
...
The application is configured to perform these steps in the background, but you can turn them off by changing the `SYNC_*` options in `prod.env`.
Also note that these sync commands will not overwrite any exercises you might have added yourself to your instance.
...
## Configuration[](https://wger.readthedocs.io/en/latest/production/docker.html#configuration "Link to this heading")
...
Now the port setting will be overwritten from the configured nginx service when you do a `docker compose up`. However, note that compose will concatenate both sets of values so in this case the application will be binded to 8080 (from the override) _and_ 80 (from the regular compose file).
...
## Deployment[](https://wger.readthedocs.io/en/latest/production/docker.html#deployment "Link to this heading")
...
There is also an example with Caddy, a webserver that can automatically generate SSL certificates for you and is very easy to use.
...
## Monitoring with grafana[](https://wger.readthedocs.io/en/latest/production/docker.html#monitoring-with-grafana "Link to this heading")
There’s a pre-configured grafana and prometheus setup that can be used to monitor the wger application as well as the logs with Loki and Alloy. To start, set the `EXPOSE_PROMETHEUS_METRICS` to true in the env file and restart the application, then go into the `grafana` folder and start the compose file.
To access the dashboards, go to <http://localhost:3000> and log in with `admin`, password `adminadmin`. To change the pre defined password, edit `grafana/web.yml`.
## Others / Common error and pitfalls[](https://wger.readthedocs.io/en/latest/production/docker.html#others-common-error-and-pitfalls "Link to this heading")
### Email verification links[](https://wger.readthedocs.io/en/latest/production/docker.html#email-verification-links "Link to this heading")
...
### CSRF errors[](https://wger.readthedocs.io/en/latest/production/docker.html#csrf-errors "Link to this heading")
You will most probably run into CSRF errors when you try to use the application, specially if you configured a domain and django’s [CSRF protection](https://docs.djangoproject.com/en/dev/ref/csrf/) kicks in. To solve this, update the env file and either
  * manually set a list of your domain names and/or server IPs `CSRF_TRUSTED_ORIGINS=https://my.domain.example.com,https://118.999.881.119:8008` If you are unsure what origin to add here, set set `DJANGO_DEBUG` to true, restart the service and the error message will tell you exactly which one django has a problem with. Note: the port is important!
  * or set the `X-Forwarded-Proto` header like in the example and set `X_FORWARDED_PROTO_HEADER_SET=True`. If you do this consult the [documentation](https://docs.djangoproject.com/en/4.1/ref/settings/#secure-proxy-ssl-header) as there are some security considerations.


### Wrong pagination links[](https://wger.readthedocs.io/en/latest/production/docker.html#wrong-pagination-links "Link to this heading")
...
Make sure that you forward the host header as well as the protocol header to the application. Consult the [Django docs](https://docs.djangoproject.com/en/6.0/ref/request-response/#django.http.HttpRequest.get_host) for details.
### Missing static files[](https://wger.readthedocs.io/en/latest/production/docker.html#missing-static-files "Link to this heading")
...
For more information, consult [django’s documentation](https://docs.djangoproject.com/en/4.1/ref/settings/#secure-proxy-ssl-header).
### Automatically start service[](https://wger.readthedocs.io/en/latest/production/docker.html#automatically-start-service "Link to this heading")
...
### Backup[](https://wger.readthedocs.io/en/latest/production/docker.html#backup "Link to this heading")
**Database volume:** The most important thing to backup. For this just make a dump and restore it when needed (you should try to run this once to make sure it works).:
```
...
**Media volume:** If you haven’t uploaded to exercises or the gallery, you don’t need to backup this, the contents can just be downloaded again. Just delete any data in the appropriate tables and run the sync commands again:
```
...
### Postgres Upgrade[](https://wger.readthedocs.io/en/latest/production/docker.html#postgres-upgrade "Link to this heading")
...
### Building the image[](https://wger.readthedocs.io/en/latest/production/docker.html#building-the-image "Link to this heading")
If you want to build your own image, you can do so by running the following commands from the server’s source folder:
```
docker build -f extras/docker/development/Dockerfile -t wger/server .

```

There is also a “base” image located in `extras/docker/base` which the server one uses as a base.
### Using sqlite[](https://wger.readthedocs.io/en/latest/production/docker.html#using-sqlite "Link to this heading")
You can also easily use sqlite as a database backend instead of postgres. If you want to use an existing database, copy it to next to the docker compose file, otherwise create an empty file, make sure the permissions are correct and then change these settings:
```
...
[](https://wger.readthedocs.io/en/latest/manual/routines.html "Using the routines") [Next ](https://wger.readthedocs.io/en/latest/production/installation.html "Manual installation")
* * *
