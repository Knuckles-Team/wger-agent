...
def agent_template(mcp_url: str = None, mcp_config: str = None, **kwargs):
    ...
    from wger_agent.graph_config import TAG_PROMPTS, TAG_ENV_VARS
        ...
        try:
            ...
            if not os.path.isabs(config_path) and "/" not in config_path:
                ...
                try:
                    ...
                    pkg_res = files("wger_agent") / config_path
                    if pkg_res.is_file():
            ...
            if os.path.exists(config_path):
                ...
                logger.info(
                    f"wger-agent: Loaded {len(mcp_toolsets)} MCP servers from {config_path}"
                )
        except Exception as e:
            logger.error(
                f"wger-agent: Failed to load MCP config {effective_mcp_config}: {e}"
            )
